...
func (r *Repository) ensureCustomFieldTables(ctx context.Context) error {
	_, err := r.db.Exec(ctx, `
		...
		CREATE UNIQUE INDEX IF NOT EXISTS idx_spare_part_field_defs_name_active
			ON spare_part_field_defs (lower(name)) WHERE active=true;
...
func (r *Repository) Update(ctx context.Context, id string, p *SparePart) error {
	_, err := r.db.Exec(ctx, `
		UPDATE spare_parts SET
		  part_number=$1, name=$2, description=$3, category=$4, manufacturer=$5,
		  manufacturer_part=$6, unit=$7, min_qty=$8, critical_qty=$9, reorder_qty=$10,
		  storage_location=$11, storage_place=$12, price=$13, infrastructure_id=$14, updated_at=NOW()
		WHERE id=$15 AND active=true`,
		p.PartNumber, p.Name, p.Description, p.Category, p.Manufacturer,
		p.ManufacturerPart, p.Unit, p.MinQty, p.CriticalQty, p.ReorderQty,
		p.StorageLocation, p.StoragePlace, p.Price, p.InfrastructureID, id)
	return err
}
...
func (r *Repository) BookMovement(ctx context.Context, m *StockMovement) error {
	...
	_, err := r.db.Exec(ctx, `UPDATE spare_parts SET stock_qty=$1, updated_at=NOW() WHERE id=$2`, newQty, m.PartID)
	if err != nil {
		return err
	}
	return r.db.QueryRow(ctx, `
...
func (r *Repository) GetMovements(ctx context.Context, partID string) ([]*StockMovement, error) {
	...
	for rows.Next() {
		m := &StockMovement{}
		if err := rows.Scan(&m.ID, &m.PartID, &m.Type, &m.Qty,
			&m.QtyBefore, &m.QtyAfter, &m.Reference, &m.Notes,
			&m.CreatedBy, &m.CreatedAt, &m.PartName, &m.UserName); err != nil {
			return nil, err
		}
...
func (r *Repository) GetFieldValues(ctx context.Context, partID string) ([]*CustomFieldValue, error) {
	...
	rows, err := r.db.Query(ctx, `
		...
		WHERE d.active=true ORDER BY d.sort_order, d.name`, partID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) UpsertFieldValues(ctx context.Context, partID string, values map[string]string) error {
	...
	for fieldID, value := range values {
		...
		if _, err := r.db.Exec(ctx, `
			INSERT INTO spare_part_field_values (part_id, field_id, value, updated_at)
			VALUES ($1, $2, $3, NOW())
			ON CONFLICT (part_id, field_id) DO UPDATE SET value=EXCLUDED.value, updated_at=NOW()`, partID, fieldID, value); err != nil {
			return err
		}
	}
	return nil
...
func partInputFromForm(r *http.Request) CreatePartInput {
	return CreatePartInput{
		PartNumber: r.FormValue("part_number"), Name: r.FormValue("name"), Description: r.FormValue("description"), Category: r.FormValue("category"),
		Manufacturer: r.FormValue("manufacturer"), ManufacturerPart: r.FormValue("manufacturer_part"), Unit: r.FormValue("unit"),
		MinQty: f64(r, "min_qty"), CriticalQty: f64(r, "critical_qty"), ReorderQty: f64(r, "reorder_qty"), InitialStock: f64(r, "initial_stock"),
		StorageLocation: r.FormValue("storage_location"), StoragePlace: r.FormValue("storage_place"), Price: f64(r, "price"),
	}
}
...
func (h *Handler) CreateField(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSONOrForm(r, &in, func() { in.Name, in.FieldType, in.SortOrder = r.FormValue("name"), r.FormValue("field_type"), int(f64(r, "sort_order")) }); err != nil { response.Error(w, 400, "ungültige eingabe"); return }
...
func (h *Handler) UpsertFieldValues(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&in); err != nil { response.Error(w, 400, "ungültige eingabe"); return }
	if err := h.svc.UpsertFieldValues(r.Context(), chi.URLParam(r, "id"), in.Values); err != nil { response.Error(w, 500, err.Error()); return }
	response.JSON(w, 200, map[string]string{"status": "gespeichert"})
}
